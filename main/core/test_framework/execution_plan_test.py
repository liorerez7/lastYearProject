...
class ExecutionPlanTest(BaseTest):
    ...
    def __init__(self, execution_plan, db_type, schema, test_name: str = "unknown"):
        ...
        self._dur, self._sels, self._sqls = defaultdict(list), {}, {}
        self.built_plan: dict[int, dict] = {}
        # self._sql_to_label: dict[str, str] = {}   # CHANGED: לא נשתמש יותר במיפוי SQL→label
    ...
    def build(self, engine, metadata):
        ...
        for idx, step in enumerate(self.plan, 1):
            ...
            # CHANGED: נשמור מידע עזר לפי label (לא חובה, אבל נוח לדיאגנוסטיקה)
            self._sqls[label] = sql
            self._sels[label] = selector

            # CHANGED: הסרה — לא שומרים map לפי SQL כדי לא להידרס
            # self._sql_to_label[sql] = label
    ...
    def run(self, engine, metadata, locust_config: dict | None = None):
        ...
        # Dispose engine after test to clean up connections
        engine.dispose()
    ...
    def _run_with_locust(self, engine, cfg: dict):
        ...
        # CHANGED: flat_queries עם מזהה צעד (step_id) כדי למנוע דריסה בין צעדים עם אותו SQL
        flat_queries = [
            (idx, step["query"])
            for idx, step in self.built_plan.items()
            for _ in range(step["repeat"])
        ...
        class DatabaseUser(User):
            ...
            @task
            def random_query(self):
                # CHANGED: בוחרים גם step_id וגם SQL
                idx, sql = random.choice(flat_queries)
                t0 = time.perf_counter()
                try:
                    self.conn.execute(text(sql))
                    duration = time.perf_counter() - t0
                ...
                # CHANGED: רושמים את המדידה תחת הלייבל של הצעד הנכון
                label = f"Step {idx}"
                parent._sqls[label] = sql
                parent._sels[label] = parent.built_plan[idx].get("selector")
                parent._dur[label].append(duration)
    ...
    def get_built_plan_with_durations(self):
        ...
        for idx, step in self.built_plan.items():
            ...
            durs = self._dur.get(label, [])
            if durs:
                durs = durs[1:]  # שים לב: זה עדיין חותך את המדידה הראשונה אם קיימות; השארתי כפי שהיה
