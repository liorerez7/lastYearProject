...
//  const [metricType, setMetricType] = useState("avg"); // "avg" | "p95" | "stddev"
...
//
//      return {
//        name: `${result.test_name} (${result.query_type})`,  // כולל query_type בהצגה
...
//      const originalData = results.find(r =>
//          `${r.test_name.length > 15 ? r.test_name.substring(0,12) + "..." : r.test_name} (${r.query_type})` === label
//        );
//      return (
...
//            {originalData?.mysql_count ?? 0} runs<br />
...
//            <div className="text-xs text-gray-600 ml-5 mt-0.5 mb-1">
//              {data.mysql_count} runs<br />
//              P95: {data.mysql_p95.toFixed(2)}s<br />
//              σ: {data.mysql_stddev.toFixed(2)}s
//          </div>
//          <div className="flex items-center text-blue-600 mb-1">
//              <div className="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
//              <span className="font-medium">MySQL:</span>
...
//                  </button>
//                  <button
//                    className={`btn btn-sm ${chartType === 'scatter' ? 'btn-primary' : 'btn-outline'}`}
...
export default function PerformanceChart({ results }) {
  ...
  const barChartData = results.map(result => {
    ...
    return {
      name: `${result.test_name} (${result.query_type})`,
      fullName: result.test_name,
      MySQL: mysqlValue,
      PostgreSQL: pgValue,
      queryType: result.query_type.replace(/_/g, " "),
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      const originalData = results.find(r =>
        `${r.test_name} (${r.query_type})` === label
      );

      return (
        <div className="p-4 border shadow-lg rounded-md bg-white opacity-95">
          <p className="text-base font-bold">{originalData?.test_name || label}</p>
          <p className="text-sm text-gray-500 mb-2 capitalize">{originalData?.query_type.replace(/_/g, ' ')}</p>
          ...
          <div className="flex items-center text-blue-600 mb-1">
            <div className="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
            <span className="font-medium">MySQL:</span>
            <span className="ml-1">{payload[0]?.value?.toFixed(3)} s</span>
          </div>

          <div className="flex items-center text-green-600 mb-1">
            <div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div>
            <span className="font-medium">PostgreSQL:</span>
            <span className="ml-1">{payload[1]?.value?.toFixed(3)} s</span>
          </div>

          {originalData?.winner !== 'Tie' && (
            <div className="mt-2 pt-2 border-t text-sm">
              <span className={`font-medium ${originalData.winner === 'MySQL' ? 'text-blue-600' : 'text-green-600'}`}>
                {originalData.winner} is {originalData.difference_percent.toFixed(1)}% faster
              </span>
